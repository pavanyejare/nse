...
url_fin      = 'https://www.nseindia.com/api/option-chain-indices?symbol=FINNIFTY'
...
def set_header():
    ...
    fin_nearest=nearest_strike_nf(fin_ul)
    ...
    for item in data['records']['data']:
        if item["expiryDate"] == currExpiryDate:
            if item["strikePrice"] == strike and item["strikePrice"] < start_strike+(step*num*2):
                #print(strCyan(str(item["strikePrice"])) + strGreen(" CE ") + "[ " + strBold(str(item["CE"]["changeinOpenInterest"]).rjust(10," ")) + " ]" + strRed(" PE ")+"[ " + strBold(str(item["PE"]["changeinOpenInterest"]).rjust(10," ")) + " ]")
    ...
    for item in data['records']['data']:
        if item["expiryDate"] == currExpiryDate:
            if item["strikePrice"] == strike and item["strikePrice"] < start_strike+(step*num*2):
                #print (item["CE"])
                #exit (0)
                if item["CE"]["changeinOpenInterest"] > max_oi:
                    max_oi = item["CE"]["changeinOpenInterest"]
...
def highest_oi_PE(num,step,nearest,url):
    ...
    for item in data['records']['data']:
        if item["expiryDate"] == currExpiryDate:
            if item["strikePrice"] == strike and item["strikePrice"] < start_strike+(step*num*2):
                if item["PE"]["changeinOpenInterest"] > max_oi:
                    max_oi = item["PE"]["changeinOpenInterest"]
                    max_oi_strike = item["strikePrice"]
                strike = strike + step
    return max_oi_strike
...
print_hr()
print_header("FinNifty",fin_ul,fin_nearest)
print_hr()
print_oi(5,50,fin_nearest,url_fin)
print_hr()
print_header("Bank Nifty",bnf_ul,bnf_nearest)
print_hr()
print_oi(5,100,bnf_nearest,url_bnf)
print_hr()
